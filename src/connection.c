...
static bool handle_irc_line(int fd, struct http_client *hc, struct bridge *br, bool *br_connected,
    ...
    /* GRAPPA admin API — intercepted here so it never falls through to
     * `handle_raw` (which would forward it to grappa as a raw IRC line,
     * which grappa would then forward upstream, which is not what a
     * `/grappa help` should do).  The `GRAPPA NETWORK <slug>` Case B
