 ...
 * nick (e.g. "SonicTest") never matches a broadcast on the folded one
 * ("sonictest"), and the mismatch is silent: no error, the frame simply
 ...
 * all, confirmed against the real SonicTest test account), unlike an
...
static bool ensure_network_connected(struct http_client *hc, const struct config *cfg,
     ...
     * (the numeric id) 404'd, even against `SonicTest`'s own
 ...
 * reproducibly, for #essency's entire topic/modes/members snapshot
 ...
 * silently lost — confirmed live: a raw-forwarded `WHOIS Sonic` produced
...
static void handle_grappa_message_event(int fd, struct bridge *br, struct grappa_session *sess,
    ...
    if (strcmp(kind, "privmsg") == 0 || strcmp(kind, "notice") == 0 ||
         ...
         * showed `* Hypnotize \x01ACTION accarezza cdc\x01\x01` as
...
static void render_names_353_366(int fd, const char *nick, const char *channel,
    ...
    for (size_t i = 0; i < count; i++) {
         ...
         * sigil — found live: `SonicTest`, a confirmed real op in
         * `#essency` (per WHOIS's own 319 channel list showing
         * `@#essency`), rendered with no `@` in `/names` at all. At
 ...
 * guessed (a raw dump showed `{"windows":{"1":[{"target_nick":"Sonic",
