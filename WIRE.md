...
- **Anything that mutates persisted/tracked session state — a chat line,
  ...
  - `PATCH /networks/:network_id` (`{"connection_state": "connected"|
    ...
    `SonicTest` session. 200 on success (whether freshly spawned or
...
**The gotcha, found live**: a `WHOIS Sonic` sent via `"raw"` produced NO
...
- `"whois_bundle"` — every field nullable, populated as 311/312/313/
  ...
  special-cased. **Confirmed live against real data**: `WHOIS Sonic` on
  azzurra correctly rendered `Sonic` as a genuine Server Administrator
...
- `"banlist_bundle"` — `{network, channel, entries: [{mask, setter,
  ...
  with no bans still gets a bare 368 — confirmed live on `#essency`).
...
into the subscription). So an unfolded join topic (`channel:SonicTest`) and
the broadcaster's folded one (`channel:sonictest`) are two different strings
...
own-nick pseudo-channel) arrives ALREADY folded (`"sonictest"`), while
`message.sender` does NOT (nick KEYs fold, but "the display stays RAW" —
confirmed live: `sender` came back exactly `"Sonic"`, mixed case). The §5
