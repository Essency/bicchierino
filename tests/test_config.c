...
TEST(a_plaintext_grappa_url_is_loopback_only) {
    ...
    /* A bracketed IPv6 loopback literal is loopback too — url_host() must
     * strip the brackets before is_loopback() ever sees the host, or this
     * wrongly refuses a genuinely-loopback URL. */
    CHECK(load("grappa-url http://[::1]:4000\nbind 127.0.0.1 6667 plain\n", &cfg, NULL, NULL, 0));
    CHECK(load("grappa-url http://[::1]\nbind 127.0.0.1 6667 plain\n", &cfg, NULL, NULL, 0));

    /* A non-loopback bracketed IPv6 literal is refused, same as any other
     * non-loopback host. */
    CHECK(!load("grappa-url http://[2001:db8::1]:4000\nbind 127.0.0.1 6667 plain\n", &cfg, NULL,
                err, sizeof(err)));
    CHECK(strstr(err, "refusing to start") != NULL);
}
